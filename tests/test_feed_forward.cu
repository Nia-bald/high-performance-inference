...
static int run_python_reference(
        int d_model, int d_ff, int batch_size, int seq_len, int seed,
        double& python_time_ms,
        std::vector<float>& h_input,
        std::vector<float>& h_W_up,
        std::vector<float>& h_b_up,
    ...
    const char* script_paths[] = {
        "../tools/python_get_feed_forward_output.py",
        "tools/python_get_feed_forward_output.py",
    };
    ...
    if (!script) {
        std::cerr << "ERROR: Cannot find python_get_feed_forward_output.py" << std::endl;
        return -1;
    }
    ...
    std::snprintf(cmd, sizeof(cmd),
        "%s %s --generate "
        "--d_model %d --d_ff %d --batch_size %d --seq_len %d --seed %d",
        python, script, d_model, d_ff, batch_size, seq_len, seed);
    ...
    // Lines 1-6: input, W_up, b_up, W_down, b_down, expected output
    parse_floats(lines[1], h_input);
    parse_floats(lines[2], h_W_up);
    parse_floats(lines[3], h_b_up);
    parse_floats(lines[4], h_W_down);
    parse_floats(lines[5], h_b_down);
    ...
    if ((int)h_W_up.size()     != DF)  { std::cerr << "W_up size mismatch\n";     return -1; }
    if ((int)h_b_up.size()     != d_ff){ std::cerr << "b_up size mismatch\n";     return -1; }
    if ((int)h_W_down.size()   != FD)  { std::cerr << "W_down size mismatch\n";   return -1; }
    if ((int)h_b_down.size()   != d_model) { std::cerr << "b_down size mismatch\n"; return -1; }
...
int main() {
    ...
    std::vector<float> h_input, h_W_up, h_b_up, h_W_down, h_b_down, h_expected;

    if (run_python_reference(D_MODEL, D_FF, BATCH_SIZE, SEQ_LEN, SEED,
                             python_time_ms,
                             h_input, h_W_up, h_b_up, h_W_down, h_b_down, h_expected) != 0) {
    ...
    // Setup memory arenas
    size_t arena_size = static_cast<size_t>(1024) * 1024 * 64; // 64 MB
    GPUMemoryArena weights_arena(arena_size);
    GPUMemoryArena inference_arena(arena_size);

    // Construct FeedForward layer
    FeedForward ffn(D_MODEL, D_FF, weights_arena);

    // Load the randomly-generated weights
    ffn.load_weights(h_W_up.data(), h_b_up.data(), h_W_down.data(), h_b_down.data());
    ...
    std::cout << "\nLaunching CUDA FeedForward forward pass..." << std::endl;
