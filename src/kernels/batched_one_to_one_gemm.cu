...
namespace kernels {
    ...
    __global__ void perform_batched_one_to_one_tiled_gmm(
        const float* A,  // [M, K]
        const float* B,  // [K, N]
        float* C, // [M, N]
        int M, int N, int K,
        ...
        for (size_t tile{}; tile <  (stride_K + TILE_SIZE-1)/TILE_SIZE; ++tile){
            ...
            if (col < N && tile*TILE_SIZE + threadIdx.y < stride_K){ 
                shared_B[threadIdx.y][threadIdx.x] = B[(B_i*stride_K + tile*TILE_SIZE + threadIdx.y)*N + col];
            }
            else{
    ...
    void launch_batched_one_to_one_gemm(
        ...
        perform_batched_one_to_one_tiled_gmm<<<gridDim, blockDim, 0, stream>>>(A, B, C, M, N, K, stride_A, stride_B, stride_K);
    ...
    __global__ void perform_batched_one_to_one_naive_gmm(
        const float* A,  // [M, K]
        const float* B,  // [K, N]
        float* C, // [M, N]
        int M, int N, int K,
        ...
        for (int k = 0; k < stride_K; ++k) {
            ...
            float b_val = B[(B_i * stride_K + k) * N + col];
            total += a_val * b_val;
        }
    ...
    void launch_batched_one_to_one_gemm_naive(
        ...
        perform_batched_one_to_one_naive_gmm<<<gridDim, blockDim, 0, stream>>>(A, B, C, M, N, K, stride_A, stride_B, stride_K);

        cudaError_t err = cudaGetLastError();
        if (err != cudaSuccess) {
            printf("CUDA Error in naive one-to-one GEMM: %s\n", cudaGetErrorString(err));
        }
    }
